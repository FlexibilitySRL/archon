...
function buildSessionConfig(
  ...
  // modelReasoningEffort comes from assistantConfig (per-provider defaults bag)
  ...
  return {
    ...
    ...(includeReasoning && modelReasoningEffort
      ? { reasoningEffort: mapReasoningEffort(modelReasoningEffort) }
      : {}),
  };
}
    ...
    try {
      if (resumeSessionId) {
        ...
        try {
          ...
          queue.push({ type: 'system', content: 'Starting fresh session after failed resume.' });
